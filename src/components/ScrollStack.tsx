...
const ScrollStack = ({
  ...
  const getElementOffset = useCallback(
    (element: HTMLElement) => {
      if (useWindowScroll) {
        ...
        // Subtract any translateY applied by this component to keep offset stable
        const index = cardsRef.current.indexOf(element as HTMLDivElement);
        if (index !== -1) {
          const lastTransform = lastTransformsRef.current.get(index);
          if (lastTransform) {
            return rawTop - lastTransform.translateY;
          }
        }
        return rawTop;
      } else {
  ...
  useLayoutEffect(() => {
    ...
    if (useWindowScroll) {
      cards = Array.from(document.querySelectorAll('.scroll-stack-card')) as HTMLDivElement[];
    } else if (scroller) {
      cards = Array.from(scroller.querySelectorAll('.scroll-stack-card')) as HTMLDivElement[];
    }
