...
export default function App() {
  ...
  const [currentView, setCurrentView] = useState<'home' | 'about' | 'consulting'>('home');
  const [aboutUsSection, setAboutUsSection] = useState<'team' | 'mission'>('team');
    ...
    <div className="min-h-screen bg-white selection:bg-sage-orange selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden lg:flex items-center space-x-8">
              {['Home', 'Courses', 'Tutorials', 'Workshops', 'Training', 'Consulting'].map((item) => (
                <a
                  key={item}
                  href={item === 'Home' || item === 'Courses' ? undefined : '#'}
                  onClick={(e) => {
                    e.preventDefault();
                    if (item === 'Home') {
                      setSelectedCategory(null);
              ...
              >
                <button
                  onClick={() => {
                    ...
                    setAboutUsSection('team');
                  }}
                  className={`text-sm font-semibold transition-colors hover:text-sage-orange cursor-pointer flex items-center space-x-1 ${
                    currentView === 'about' ? 'text-sage-orange' : 'text-white/90'
                  }`}
                >
                  <span>About Us</span>
                  <ChevronDown size={14} className={`transition-transform duration-200 ${isAboutDropdownOpen ? 'rotate-180 text-sage-orange' : ''}`} />
                </button>
                ...
                <AnimatePresence>
                  {isAboutDropdownOpen && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 mt-2 w-52 bg-sage-navy border border-white/10 rounded-xl shadow-2xl py-2 z-50 overflow-hidden"
                    >
                      <a
                        href="#team"
                        onClick={(e) => {
                        ...
                        className="block px-4 py-2.5 text-xs font-bold text-white/90 hover:text-sage-orange hover:bg-white/5 transition-colors cursor-pointer"
                      >
                        SAGE Team
                      </a>
                      <a
              ...
              <div className="px-4 pt-2 pb-6 space-y-1">
                {['Home', 'Courses', 'Tutorials', 'Workshops', 'Training', 'Consulting'].map((item) => (
                  <a
                    key={item}
                    href={item === 'Home' || item === 'Courses' ? undefined : '#'}
                    className="block px-3 py-3 text-base font-medium text-white hover:bg-white/10 rounded-md cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      setIsMobileMenuOpen(false);
                      if (item === 'Home') {
                ...
                {/* Mobile About Us Dropdown Group */}
                <div className="pt-2 border-t border-white/10">
                  <div className="px-3 py-2 text-xs font-bold uppercase tracking-wider text-sage-orange">
                    About Us
                  </div>
                  <a
                    href="#team"
                    className="block px-6 py-2 text-sm text-white/90 hover:text-sage-orange hover:bg-white/5 rounded-md cursor-pointer"
                    onClick={(e) => {
                      e.preventDefault();
                      setIsMobileMenuOpen(false);
                      setSelectedCategory(null);
      ...
      {selectedCategory !== null ? (
        ...
        <ConsultingView
          ...
          onSelectTrackContact={(trackTitle) => {
            setToastMessage(`Inquiry started for ${trackTitle}. Contact info@shastryassociates.com or call +1 (555) 123-4567.`);
